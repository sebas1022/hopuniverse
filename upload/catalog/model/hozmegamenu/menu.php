...
class ModelHozmegamenuMenu extends Model {
	 ...
	 public function getMenuCustomerLink($lang_id = NULL,$setting = array()) {
        ...
        if ($showhome) {
		   ...
		   if(isset($this->request->get['route'] )) {
            ...
            $id = "_home";
            $menu_link .= '<div id="pt_menu' . $id . '" class="pt_menu' . $act . '">';
            $menu_link .= '<div class="parentMenu">';
            $menu_link .= '<a href="'.$url.'">';
            $menu_link .= '<span>' . $this->language->get("text_home"). '</span>';
            $menu_link .= '</a>';
            $menu_link .= '</div>';
            $menu_link .= '</div>';
        }
		...
		foreach ($menu_items as $item)
		{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				case 'CAT':
					$item1 = $item1+ 1; 
					$menu_link .= $this->drawCustomMenuItem($id, 0, false, $item1, $lang_id,$setting);
					break;
				case 'LINK': 
					...
					if($link) {
						...
						$menu_link .= '<div id ="pt_menu_link' . $id . '" class ="pt_menu"><div class="parentMenu" ><a href="' . ($link['link']) . '"><span>' . $link['title'] . '</span></a></div></div>' . PHP_EOL;
					}
				case 'CMS': 
                    ...
                    if(isset($cms['information_id'])) {
    					...
    					$menu_link .= '<div id ="pt_menu_cms" class ="pt_menu"><div class="parentMenu" ><a href="'.$cms_link.'"><span>'.$cms['title'].'</span></a></div></div>'.PHP_EOL;
                    }
			}
		...
		foreach ($blockCustomer as $bc) {
			...
			if($bc['status'] && $flag) {
				$menu_link .= $this->drawCustomMenuBlock($bc['identify'], $bc);
			}
        }
	...
	public function getMenuCustomerLinkMobile($lang_id = NULL,$setting = array()) {
		...
		$item1=0;
		$lang_id = (int)$this->config->get('config_language_id');
		$menu_link = '<ul id="ma-mobileSubMenu" class="mobileSubMenu nav-collapse collapse">';
		...
		foreach ($menu_items as $item)
		{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				case 'CAT':
					...
					if($curr_cate) {
						$item1 = $item1+ 1; 
						$menu_link .= $this->drawCustomMenuItemMobile($id, 0, false, $item1, $lang_id,$setting);
				...
				case 'LINK': 
					...
					if(isset($link[0])) {
						...
						if($link_show) {
							$menu_link .= '<li><a href="' . ($link['link']) . '"><span>' . $link['title'] . '</span></a></li>' . PHP_EOL;
						}
					}
					break;
				case 'CMS':
                    ...
                    if(isset($cms['information_id'])) {
    					$cms_link = $this->url->link('information/information', 'information_id=' . $cms['information_id']);
    					$menu_link .= '<li><a href="'.$cms_link.'"><span>'.$cms['title'].'</span></a></li>'.PHP_EOL;
                    }
					break;
			}
...
//        $html[] = '</a>';
        ...
        if ($drawPopup) {
            ...
            $html[] = '<div id="SubMenu' . $id . '" class="SubMenu cmsblock" style="display: none; width: 904px;">';
            if ($blockHtml) {
	...
	public function drawCustomMenuItem($category, $level = 0, $last = false, $item, $lang_id,$setting) {
        ...
        if ($drawPopup) {
            $html[] = '<div id="pt_menu' . $id . '" class="pt_menu' . $active . ' nav-' . $item . ' has_children">';
        } else {
            $html[] = '<div id="pt_menu' . $id . '" class="pt_menu' . $active . ' nav-' . $item . ' pt_menu_no_children">';
        }
		//echo $category;
        ...
        if ($drawPopup) {
            if ($this->_show_level >= 2) {
                // --- Popup function for hide ---
                $html[] = '<div id="SubMenu' . $id . '" class="SubMenu" style="display: none; width: 1228px;"><div class="SubMenu-content">';
                // --- draw Sub Categories ---
                 if (count($activeChildren) || $blockHtml) {
					//echo "<pre>"; print_r($blockHtml); echo "</pre>"; 
                    $html[] = '<div class="SubMenu-section SubMenu-nav block block1 " id="block1' . $id . '">';
                ...
                if ($blockHtml && !$blockHtmlRight) { 
                    $html[] = '<div class="block block2" id="block2' . $id . '">';
                    $html[] = $blockHtml;
                    $html[] = '</div>';
                }
                $html[] = '</div></div>';
