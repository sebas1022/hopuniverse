...
class ControllerBlogArticle extends Controller {
	public function index() {
		...
		if ($article_info) {
			...
			$image_size['width'] = $this->config->get('module_ocblog_article_width');
			$image_size['height'] = $this->config->get('module_ocblog_article_height');

			if($image_size['width'] && $image_size['height']) {
				$data['image'] = $this->model_tool_image->resize($article_info['image'], $image_size['width'], $image_size['height']);
			} else {
				$data['image'] = $this->model_tool_image->resize($article_info['image'], 1170, 700);
			}
